...
describe("Router", () => {
  ...
  it("uses the handler with the same pattern", async () => {
    const router = new Router();
    router.get("/test1", (req) => new Response("test1"));
    router.get("/test2", (req) => new Response("test2"));
    const { fetchText } = await createTestServer(
      router.handleRequest.bind(router),
    );
  ...
  it("handles multiple methods", async () => {
    const router = new Router();
    router.get("/test", (req) => new Response("GET"));
    router.post("/test", (req) => new Response("POST"));
    const { fetchText } = await createTestServer(
      router.handleRequest.bind(router),
    );
